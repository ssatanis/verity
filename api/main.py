...
"""
import json, os, sys, uuid, secrets, datetime as dt
...
def clusters(limit: int = 50, state: str | None = None):
    with pg() as c:
        return c.execute("SELECT id, rank, score, state, city, county, n_providers, n_hospice, n_hha, n_snf, dollars_at_risk, summary FROM public.clusters WHERE eligible AND (%s IS NULL OR state = %s) ORDER BY rank LIMIT %s", (state, state, limit)).fetchall()
...
def review(req: ReviewReq):
    if req.decision not in ("accept", "reject", "needs_info"): raise HTTPException(400)
    with pg() as c:
...
def bb_callback(code: str | None = None, state: str | None = None, error: str | None = None):
    ...
    tok = bb.exchange_code(code, verifier); token = tok["access_token"]; patient_id = tok.get("patient")
