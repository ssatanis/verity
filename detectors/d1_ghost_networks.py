...
Graph: hospice, home health agency and skilled nursing facility enrollments (CMS PECOS public files) linked to
  ...
  - practice, mailing and owner addresses (USPS-normalized; unit-level and building-level keys),
...
# org owners: PECOS ID, then normalized name + state, then fuzzy token-set within state
...
log(f"org owner records {len(org):,} -> {org.org_id.nunique():,} organizations; persons -> {per.person_id.nunique():,}")
...
for ci, c in enumerate(comps):
    ...
    # one date per distinct organization (several enrollments of one legal entity share an incorporation date), recent = 2019 onward
    org_dates = {}
    for d in pd_:
