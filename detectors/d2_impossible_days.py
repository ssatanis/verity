...
**Results.** {imp[0]:,} rendering NPIs have at least one impossible month ({imp[1]:,} impossible by line count alone, {imp[2]:,} with three or more impossible months), with ${imp[3]:,.2f}M paid in those months; {imp[5]:,} of them are tier A (an individual whose impossible personal-service hours were billed by three or more different organizations in the same month, or an organization with more than 24 hours per patient per day); {imp[4]:,} Minnesota providers exceed the state's own daily cap allowance. The most common drivers are office E/M codes (99213, 99214) and psychotherapy (90837, 90791), which several states let clinics bill under a supervising physician's or psychologist's NPI; a single-organization impossibility is therefore tier B and reads as "verify the state's supervisory billing rule and pull records", not as a finding.
