...
SELECT *, LEAST(100.0, (CASE tier WHEN 1 THEN 90 WHEN 2 THEN 75 WHEN 3 THEN 60 WHEN 4 THEN 45 ELSE 25 END) + LEAST(16, 8 * GREATEST(n_strong - 1, 0)) + LEAST(9.0, LOG10(GREATEST(dollars_at_risk, 1))) + procedure_points) AS score,
       ...
       concat_ws('; ',
         CASE WHEN d3_a = 1 THEN 'Listed on ' || pretty_source(d3_sources) || ' since ' || strftime(d3_first_event, '%B %-d, %Y') || '; Medicaid still paid claims in ' || d3_months || CASE WHEN d3_months = 1 THEN ' later month, $' ELSE ' later months, $' END || format('{:,}', CAST(ROUND(d3_paid_after) AS BIGINT)) || ' in total' END,
         ...
         CASE WHEN d2_tier = 'A' THEN 'Billed more hands-on hours than a day holds in ' || months_impossible || CASE WHEN months_impossible = 1 THEN ' month, peaking at ' ELSE ' months, peaking at ' END || ROUND(peak_hours_per_day, 1) || ' hours per day across ' || max_billing_orgs || ' billing organizations' END,
         CASE WHEN d2_tier = 'B' AND d2_umbrella THEN 'Hours beyond a day in ' || months_impossible || CASE WHEN months_impossible = 1 THEN ' month, but with up to ' ELSE ' months, but with up to ' END || CAST(max_patients AS BIGINT) || ' patients a month' || CASE WHEN max_billing_orgs >= 3 THEN ' across ' || max_billing_orgs || ' billing organizations' ELSE '' END || ', which points to a supervising clinician on the claims rather than one person''s hours; records needed'
              WHEN d2_tier = 'B' THEN 'Hours beyond a day in ' || months_impossible || CASE WHEN months_impossible = 1 THEN ' month' ELSE ' months' END || ' under one organization, which can be supervisory billing; records needed' END,
         CASE WHEN months_over_mn_cap > 0 THEN 'Over the state daily cap in ' || months_over_mn_cap || CASE WHEN months_over_mn_cap = 1 THEN ' month' ELSE ' months' END END,
