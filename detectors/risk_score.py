...
SELECT u.npi, COALESCE(n.org_name, trim(COALESCE(n.first_name,'') || ' ' || COALESCE(n.last_name,''))) AS name, n.entity_type, n.city, n.state, n.taxonomy, pc.county_fips, ps.state AS medicaid_state,
       ...
       GREATEST(COALESCE(d3.d3_paid_after, 0), COALESCE(d2.paid_flagged_months, 0), CASE WHEN d1.eligible THEN COALESCE(d1.medicaid_2024, 0) ELSE 0 END) AS dollars_at_risk,
...
con.execute("""
CREATE OR REPLACE TABLE provider_risk AS
SELECT *, LEAST(100.0, (CASE tier WHEN 1 THEN 90 WHEN 2 THEN 75 WHEN 3 THEN 60 WHEN 4 THEN 45 ELSE 25 END) + LEAST(16, 8 * GREATEST(n_detectors - 1, 0)) + LEAST(9.0, LOG10(GREATEST(dollars_at_risk, 1)))) AS score,
       ...
       concat_ws('; ',
         ...
         CASE WHEN n_detectors >= 2 THEN 'reached independently by ' || n_detectors || ' detectors' END) AS reasons
FROM provider_risk""")
...
S = {}
S["tiers"] = q("SELECT tier, tier_label, COUNT(*), ROUND(SUM(dollars_at_risk)/1e6,2), COUNT(*) FILTER (WHERE n_detectors >= 2) FROM provider_risk GROUP BY 1,2 ORDER BY 1")
...
S["corroborated"] = q("SELECT COUNT(*) FROM provider_risk WHERE n_detectors >= 2")
