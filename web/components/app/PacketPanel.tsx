    ...
    <div className="card p-5">
      ...
      {p && (
        <div className="mt-4 text-[13.5px]">
          ...
          <div className="mt-4"><div className="eyebrow mb-1">In plain language</div><p className="leading-6">{p.plain_english}</p></div>
          <div className="mt-4"><div className="eyebrow mb-2">What the records show</div>
            <ol className="space-y-2">{p.findings?.map((f: any, i: number) => <li key={i} className="flex gap-3"><span className="serif text-[17px] shrink-0" style={{ color: "var(--blue)" }}>{String(i + 1).padStart(2, "0")}</span><span>{f.text} <span className="text-[var(--ink-3)] text-[11px]">sources {f.evidence_ids?.map((n: number) => n + 1).join(", ")}</span></span></li>)}</ol></div>
          <div className="mt-4"><div className="eyebrow mb-1">Regulations this relates to</div>{p.grounds?.map((g: any) => <div key={g.cfr} className="py-1.5 rule"><span className="font-semibold">42 CFR {g.cfr}</span> <span className="text-[var(--ink-2)]">{g.text}</span></div>)}</div>
