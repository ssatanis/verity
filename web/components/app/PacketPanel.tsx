      ...
      {p && (
        <div className="mt-4 text-[13.5px]">
          ...
          {p.procedures?.length ? <div className="mt-4"><div className="eyebrow mb-2">Procedures behind the dollars</div>
            <table className="table"><thead><tr><th>program</th><th>code</th><th>what it is</th><th>paid</th><th>share</th><th>comparison</th></tr></thead>
              <tbody>{p.procedures.map((c: any, i: number) => <tr key={i}><td className="text-[11px]">{c.program}</td><td className="mono">{c.code}</td><td className="text-[12px] max-w-[260px]">{c.description || ""}{c.high_vector ? <span className="tag tag-danger ml-2">history of abuse</span> : null}</td><td>{money(c.paid)}</td><td><div className="flex items-center gap-2"><div className="w-[60px] h-[6px] bg-[var(--paper-2)]"><div className="h-[6px]" style={{ width: `${Math.max(2, Number(c.share) * 100)}%`, background: "var(--blue)" }} /></div><span className="text-[11px]">{Math.round(Number(c.share) * 100)}%</span></div></td><td className="text-[11.5px] text-[var(--ink-2)]">{c.percentile != null ? `${ordinal(Number(c.percentile) * 100)} percentile of providers on this code (${moneyExact(c.per_patient_month)} per patient-month, typical ${moneyExact(c.typical)})` : c.charge_ratio != null ? `charge to allowed ${Number(c.charge_ratio).toFixed(1)}x${c.peer_ratio != null ? `, usual ${Number(c.peer_ratio).toFixed(1)}x` : ""}` : ""}</td></tr>)}</tbody></table></div> : null}
          {p.caveats?.length ? <div className="mt-4"><div className="eyebrow mb-1">Rule out first</div><ul className="space-y-1.5 text-[13px] text-[var(--ink-2)]">{p.caveats.map((c: string, i: number) => <li key={i} className="flex gap-2"><span style={{ color: "var(--blue)" }}>,</span><span>{c}</span></li>)}</ul></div> : null}
