  ...
  return (
    <header className="bg-white sticky top-0 z-30">
      <div className="max-w-[1860px] mx-auto px-6 md:px-[72px] h-[76px] md:h-[92px] flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 text-[15px]"><Link href="/app" className="hover:text-[var(--blue)]">Console Login</Link></div>
      ...
      {open && <div className="md:hidden px-6 pb-6 flex flex-col gap-4 text-[16px] bg-white" style={{ borderTop: "1px solid var(--line)" }}>{links.map(([n, h]) => <a key={n} href={h} onClick={() => setOpen(false)} className="pt-4">{n}</a>)}<Link href="/app" className="pt-4">Console Login</Link></div>}
    </header>
  );
}
