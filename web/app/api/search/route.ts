...
export async function GET(req: Request) {
  const raw = (new URL(req.url).searchParams.get("q") ?? "").trim(); if (raw.length < 2) return NextResponse.json({ hits: [] });
  ...
  if (P.npi) {
    ...
    return NextResponse.json({ hits, parsed: P });
  }
  ...
  if (isNpiPrefix) {
    ...
    for (const r of risk ?? []) push(r, "risk"); for (const r of prov ?? []) push(r, "providers");
    return NextResponse.json({ hits: hits.slice(0, 12), parsed: P });
  }
  const name = P.name; if (name.length < 2) return NextResponse.json({ hits: [], parsed: P });
  ...
  return NextResponse.json({ hits: hits.slice(0, 14), parsed: P });
}
