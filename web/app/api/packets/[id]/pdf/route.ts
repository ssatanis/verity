...
function fonts() {
  ...
  Font.register({ family: "Caslon", fonts: [{ src: F("eb-garamond/files/eb-garamond-latin-400-normal.woff") }, { src: F("eb-garamond/files/eb-garamond-latin-600-normal.woff"), fontWeight: 700 }] });
  Font.register({ family: "Inter", fonts: [{ src: F("open-sans/files/open-sans-latin-400-normal.woff") }, { src: F("open-sans/files/open-sans-latin-600-normal.woff"), fontWeight: 600 }] });
  Font.registerHyphenationCallback(w => [w]);
}
const S = StyleSheet.create({
  page: { paddingTop: 48, paddingBottom: 56, paddingHorizontal: 52, fontFamily: "Inter", fontSize: 9.5, color: "#000000", lineHeight: 1.45 },
  eyebrow: { fontSize: 8, letterSpacing: 1.2, textTransform: "uppercase", color: "#002856", fontWeight: 600 },
  h1: { fontFamily: "Caslon", fontSize: 30, lineHeight: 1.05, marginTop: 8 },
  h2: { fontFamily: "Caslon", fontSize: 17, marginTop: 18, marginBottom: 6, borderBottom: "1 solid #000000", paddingBottom: 3 },
  meta: { fontSize: 8, color: "#6b6b70", marginTop: 6 },
  p: { marginBottom: 6 }, small: { fontSize: 8, color: "#48484d" },
  row: { flexDirection: "row", marginBottom: 4 }, num: { width: 22, fontFamily: "Caslon", fontSize: 12, color: "#002856" }, cell: { flex: 1 },
  cite: { fontSize: 7.5, color: "#6b6b70" },
  box: { border: "1 solid #d9dde3", padding: 10, marginTop: 6, backgroundColor: "#f5f5f5" },
  footer: { position: "absolute", bottom: 24, left: 52, right: 52, flexDirection: "row", justifyContent: "space-between", fontSize: 7.5, color: "#6b6b70", borderTop: "1 solid #d9dde3", paddingTop: 6 },
});
...
function Doc({ p, id, status }: { p: any; id: string; status: string }) {
  ...
  return h(Document, { title: clean(p.title), author: "Verity", subject: "Referral candidate packet" },
    h(Page, { size: "LETTER", style: S.page },
      ...
      ...(p.evidence ?? []).map((e: any) => h(View, { style: S.row, key: "e" + e.id, wrap: false }, h(Text, { style: { width: 26, fontSize: 8, color: "#002856" } }, `[${e.id}]`), h(View, { style: S.cell }, h(Text, { style: { fontSize: 8.5 } }, clean(e.statement)), h(Text, { style: S.cite }, clean(e.source))))),
