...
function Doc({ p, status }: { p: any; status: string }) {
  ...
  return h(Document, { title: clean(p.title), author: "Verity", subject: "Referral candidate packet" },
    h(Page, { size: "LETTER", style: S.page },
      ...
      ...(p.grounds ?? []).map((g: any) => h(View, { style: S.gRow, key: g.cfr }, h(Text, { style: S.gCode }, `42 CFR ${g.cfr}`), h(Text, { style: S.cell }, clean(g.text)))),
      ...
      h(View, { style: S.footer, fixed: true }, h(Text, null, "Verity  ·  indicators from public records, not findings  ·  ss4497@cornell.edu"), h(Text, { render: ({ pageNumber, totalPages }: any) => `Page ${pageNumber} of ${totalPages}` })),
    ));
}
