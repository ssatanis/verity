...
import { sourceName, labelName, idMatchName, money, revocationReason, titleCase } from "@/lib/labels";
      ...
      <div className="grid md:grid-cols-[1.4fr_1fr] gap-6 mt-8 items-start">
        <div className="space-y-6 min-w-0">
          {(rev?.length || leie?.length) ? <div className="card p-5 overflow-x-auto"><h2 className="serif text-[24px] mb-2">Public list actions</h2>
            {rev?.map((r, i) => <div key={i} className="text-[13px] py-2 rule"><span className="font-semibold">Medicare revocation</span> effective {r.revoked_dt}, barred from re-enrolling until {r.reenroll_bar_dt}<div className="text-[12px] text-[var(--ink-2)]">{revocationReason(r.revocation_rsn)}, {titleCase(r.provider_type_desc)}, {r.state}</div></div>)}
            {leie?.map((r, i) => <div key={i} className="text-[13px] py-2 rule"><span className="font-semibold">OIG exclusion</span> {r.excl_dt} under section {r.excltype}{r.rein_dt ? `, reinstated ${r.rein_dt}` : ""}<div className="text-[12px] text-[var(--ink-2)]">{titleCase(r.general)}, {titleCase(r.specialty)}, {titleCase(r.city)}, {r.state}</div></div>)}
          </div> : null}
          {d3.length ? <div className="card p-5 overflow-x-auto"><h2 className="serif text-[24px] mb-1">Paid after the action</h2><p className="text-[12.5px] text-[var(--ink-3)] mb-2">Months in which Medicaid paid claims for this provider after a public action that should have triggered a screening check. This is a record of payments, not a determination that they were improper.</p>
            <table className="table"><thead><tr><th>list</th><th>identity check</th><th>action date</th><th>window closed</th><th>months paid after</th><th>first</th><th>last</th><th>paid after</th><th>12 months before</th></tr></thead>
              <tbody>{d3.map(f => { const e = ev(f.evidence); return <tr key={String(f.id)}><td>{sourceName(e.source)}<div className="text-[11px] text-[var(--ink-3)] max-w-[260px]">{revocationReason(e.reason)}</div></td><td className="text-[11px]">{idMatchName(e.id_match)}</td><td>{e.event_dt}</td><td>{e.window_end ?? "still open"}</td><td>{e.months_paid_after}</td><td>{e.first_month_after}</td><td>{e.last_month_after}</td><td>{money(e.paid_after)}</td><td>{money(e.paid_before_12m)}</td></tr>; })}</tbody></table>
          </div> : null}
