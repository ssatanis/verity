...
export default async function Flags({ searchParams }: { searchParams: Promise<{ detector?: string; state?: string; tier?: string }> }) {
  ...
  return (
    <div>
      ...
      <p className="text-[13.5px] text-[var(--ink-2)] mt-4 mb-5 max-w-3xl leading-6">{detector === "D3" ? "Providers on a Medicare revocation, OIG exclusion, SAM.gov or state exclusion list whom Medicaid kept paying after the action. Tier A means the reason for the action concerns integrity, the NPI passes its check digit, and the name on the list agrees with the national registry. The dollars are payments after an action that should have triggered a screening check, not a determination that they were improper." : "Clinicians whose Medicaid billing implies more hours of hands-on care than a day holds, organisations billed for more than 24 hours per patient per day, and providers over a state's own daily cap. Tier A needs the hours to come from three or more billing organisations in the same month, a per-patient breach, or a cap breach. Hours beyond a day under a single organisation are tier B, because several states let clinics bill under a supervising clinician's NPI."}</p>
      <div className="flex flex-wrap gap-2 mb-5 text-[12px]">{[["D3", "Paid after a list action"], ["D2", "Hours per day"]].map(([d, n]) => <Link key={d} href={`/app/flags?detector=${d}`} className={`tag ${detector === d ? "tag-ink" : ""}`}>{n}</Link>)}
