...
export default async function Candidates({ searchParams }: { searchParams: Promise<{ state?: string; tier?: string; detector?: string; page?: string }> }) {
  ...
  return (
    <div>
      ...
      <h1 className="display serif text-[40px] md:text-[54px] mt-2">Every flagged provider, ranked.</h1>
      ...
      <div className="text-[11.5px] text-[var(--ink-3)] mt-3">{tier ? TIER_LABEL[Number(tier)] : "Tiers: 1 on a public list and still paid; 2 more hours than a day holds across organisations; 3 part of a network that touches a public list; 4 network structure alone or hours beyond a day under one organisation; 5 worth knowing."}</div>
