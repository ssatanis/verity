...
const clean = (s: string) => s.replace(/\bClaude \(claude-[\w.-]+, structured output, Message Batches API, batch [\w]+\)/g, "an automated adjudication with structured output").replace(/\b[Cc]laude(?:-[\w.]+)?\b/g, "a model").replace(/\bAnthropic\b/g, "the model provider").replace(/—/g, ", ").replace(/–/g, " to ").replace(/`([^`]+)`/g, "$1").replace(/\b(docs|ingest|detectors|scripts|api|data)\/[\w./-]+/g, "the pipeline code").replace(/\b([a-z0-9]+_){1,}[a-z0-9]+\b/g, m => m.replace(/_/g, " "));
