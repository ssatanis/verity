...
import { FactorDesk } from "@/components/app/FactorDesk";
export const revalidate = 60;
export default async function Cluster({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params; const sb = publicClient();
  const [{ data: c }, { data: members }, { data: packets }, { data: factors }] = await Promise.all([
    ...
    sb.from("network_factors").select("*").eq("cluster_id", id),
  ]);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row items-start justify-between gap-6 mt-2 mb-6">
        <div className="max-w-3xl"><h1 className="display serif text-[34px] md:text-[42px]">{c.id} <span className="text-[var(--ink-3)]">{c.city}</span></h1><p className="serif text-[22px] mt-3" style={{ color: "var(--blue)" }}>{c.summary}</p>
      ...
      <div className="mb-6"><FactorDesk rows={(factors as any) ?? []} /></div>
