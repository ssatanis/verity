...
export default async function Clusters({ searchParams }: { searchParams: Promise<{ state?: string; all?: string }> }) {
  ...
  return (
    <div>
      ...
      <Filters query={{ state, all }} path="/app/clusters" resetTo="/app/clusters" groups={[
        { name: "state", chips: [{ key: "state", label: "All states" }, ...counts.map(([s, n]: any) => ({ key: "state", value: String(s), label: String(s), count: Number(n) }))] },
        { name: "chains", chips: [{ key: "all", label: "Ranked networks only" }, { key: "all", value: "1", label: "Include chains", accent: true, title: "Also show networks that are mostly part of a known chain or private equity group" }] },
      ]} />
      <div className="flex flex-wrap gap-2 mt-3 mb-5 text-[12px]"><Link href="/app/plazas" className="tag">Addresses hosting many providers</Link></div>
      ...
      <div className="card overflow-x-auto"><table className="table">
        ...
        <tbody className="rows-in">{data?.map(c => { const f = (typeof c.features === "string" ? JSON.parse(c.features) : c.features) ?? {}; const facts: string[] = f.facts ?? []; return <tr key={c.id}><td>{c.rank}</td><td><Link href={`/app/clusters/${c.id}`} className="link serif text-[16px]">{c.summary}</Link><div className="text-[11px] text-[var(--ink-3)]">{c.id}{c.chain_or_pe ? ", chain" : ""}</div></td><td>{c.n_providers}</td><td style={{ color: "var(--blue)" }}>{Number(c.score).toFixed(0)}</td><td>{money(c.dollars_at_risk)}</td><td className="max-w-[460px] text-[12px] text-[var(--ink-2)]"><ul className="space-y-1">{facts.slice(0, 3).map((x, i) => <li key={i}>{x}</li>)}</ul></td></tr>; })}</tbody>
      </table></div>
      {(data?.length ?? 0) === 0 && <div className="card-2 p-6 mt-4 text-[13px]">No network matches these filters. Clear the state, or turn on "include chains" to see networks that are mostly part of a known chain.</div>}
      </div>
      {plazas?.length ? <div className="card p-5 mt-6"><div className="flex justify-between items-center"><h2 className="serif text-[24px]">Addresses hosting many providers</h2><Link href="/app/plazas" className="link text-[12px]">All addresses</Link></div><p className="text-[12.5px] text-[var(--ink-3)] mt-1 mb-2">One Van Nuys office plaza hosts dozens of hospice enrollments. Large medical buildings appear here too; an address is a lead, not a finding.</p>
        <table className="table"><thead><tr><th>address</th><th>providers</th><th>formed since 2019</th><th>on a list</th><th>revoked company here</th></tr></thead><tbody>{plazas.map((r: any) => <tr key={r.address}><td className="mono">{String(r.address).split("|")[0]}, {titleCase(r.city)} {r.state}</td><td>{r.n_providers}</td><td>{r.n_since_2019}</td><td>{r.n_labelled}</td><td>{r.revoked_entity_here ? "yes" : ""}</td></tr>)}</tbody></table></div> : null}
    </div>
  );
}
