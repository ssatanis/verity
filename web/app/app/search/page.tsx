...
export default async function Search({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  let registry: any[] = [];
  if (term) { try { registry = (await nppesSearch(term, 25)).filter((h: any) => !seen.has(h.npi)); } catch {} }
  return (
    <div>
      <div className="eyebrow">Search</div>
      <h1 className="display serif text-[34px] md:text-[44px] mt-2">{term ? <>Results for <span style={{ color: "var(--blue)" }}>{term}</span></> : "Look up any provider in the country"}</h1>
