...
export default async function StatePage({ params }: { params: Promise<{ st: string }> }) {
  ...
  return (
    <div>
      <div className="eyebrow">State view</div><h1 className="display serif text-[54px] mt-2">{S}: show the math</h1>
      <p className="text-[13px] text-[var(--ink-3)] mt-2 max-w-3xl">Dollars by detector for the counties in this state, one figure per provider, every figure traceable to a public row. Sums are flagged Medicaid dollars, not estimates of loss.</p>
      <div className="grid md:grid-cols-3 gap-4 mt-6 mb-6">
        {[["Ghost network communities (Medicaid 2024)", tot.d1], ["Impossible days (paid in flagged months)", tot.d2], ["Paid after a screening trigger", tot.d3]].map(([l, v]) => <div key={String(l)} className="kpi"><div className="eyebrow">{l}</div><div className="serif text-[40px] mt-2">{money(v as number)}</div></div>)}
      </div>
      {S === "MN" && (
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          <div className="card p-5 overflow-x-auto"><h2 className="serif text-[24px] mb-1">EIDBI, CTSS, HSS and PCA codes</h2><p className="text-[12px] text-[var(--ink-3)] mb-2">Rendering NPIs, Medicaid dollars 2018 to 2024, implied hours, and where the unit price came from.</p>
            <table className="table"><thead><tr><th>code</th><th>NPIs</th><th>$M</th><th>implied hours (M)</th><th>rate source</th></tr></thead><tbody>{mnCodes.map((r: any[]) => <tr key={r[0]}><td>{r[0]}</td><td>{num(Number(r[1]))}</td><td>{r[2]}</td><td>{r[3]}</td><td className="text-[11px]">{r[4]}</td></tr>)}</tbody></table></div>
          <div className="card p-5 overflow-x-auto"><h2 className="serif text-[24px] mb-1">Unit price validation</h2><p className="text-[12px] text-[var(--ink-3)] mb-2">Data-driven estimates against Minnesota's published DHS-3945 and EIDBI rates. Positive error means the estimate sits above the published price, which keeps implied hours conservative.</p>
            <table className="table"><thead><tr><th>estimator</th><th>median signed error</th><th>median abs error</th><th>code-years</th></tr></thead><tbody>{val.map((r: any[]) => <tr key={r[0]}><td>{r[0]}</td><td>{r[1] ?? ""}{r[1] != null ? "%" : ""}</td><td>{r[2] ?? ""}{r[2] != null ? "%" : ""}</td><td>{r[3]}</td></tr>)}</tbody></table></div>
        </div>
      )}
      <div className="grid md:grid-cols-2 gap-6">
        <div className="card p-5"><h2 className="serif text-[24px] mb-2">Counties</h2><table className="table"><thead><tr><th>county</th><th>communities</th><th>flagged NPIs</th><th>at risk</th></tr></thead><tbody>{counties?.map(c => <tr key={c.county_fips}><td>{c.county_name}</td><td>{c.n_clusters}</td><td>{c.n_providers_flagged}</td><td>{money(c.dollars_at_risk)}</td></tr>)}</tbody></table></div>
        <div className="card p-5"><h2 className="serif text-[24px] mb-2">Provider communities</h2>{clusters?.map(c => <Link key={c.id} href={`/app/clusters/${c.id}`} className="block py-2 rule text-[13px]"><span className="font-medium">{c.id} · {c.city}</span> <span className="text-[var(--ink-3)]">{Number(c.score).toFixed(1)} · {money(c.dollars_at_risk)}</span><div className="text-[12px] text-[var(--ink-2)] line-clamp-2">{c.summary}</div></Link>)}</div>
        <div className="card p-5 overflow-x-auto"><h2 className="serif text-[24px] mb-2">Paid after a screening trigger</h2><table className="table"><thead><tr><th>provider</th><th>source</th><th>action</th><th>paid after</th></tr></thead><tbody>{d3?.map(f => { const e = ev(f.evidence); return <tr key={String(f.id)}><td><Link href={`/app/providers/${f.npi}`} className="link">{f.name || f.npi}</Link></td><td className="text-[11px]">{e.source}</td><td>{e.event_dt}</td><td>{money(e.paid_after)}</td></tr>; })}</tbody></table></div>
        <div className="card p-5 overflow-x-auto"><h2 className="serif text-[24px] mb-2">Impossible days</h2><table className="table"><thead><tr><th>provider</th><th>month</th><th>label</th><th>h/day</th><th>paid</th></tr></thead><tbody>{d2rows.map(f => { const e = ev(f.evidence); return <tr key={String(f.id)}><td><Link href={`/app/providers/${f.npi}`} className="link">{f.name || f.npi}</Link></td><td>{String(f.month).slice(0, 7)}</td><td className="text-[11px]">{e.label}</td><td>{Number(f.value).toFixed(1)}</td><td>{money(e.paid)}</td></tr>; })}</tbody></table></div>
      </div>
    </div>
  );
}
