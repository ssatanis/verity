...
export async function evidenceLines(sb: SupabaseClient, kind: "cluster" | "provider", id: string): Promise<{ lines: Line[]; types: string[] }> {
  ...
  if (risk) L.push(["provider_risk", `Evidence tier ${risk.tier} (${risk.tier_label}); detectors ${(risk.detectors ?? []).join(", ")}; dollars at risk ${$(risk.dollars_at_risk)} (highest single-detector figure, not a sum); reasons: ${risk.reasons}.`]);
