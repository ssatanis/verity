...
**In plain language.** This detector builds a network linking 31,706 skilled nursing, home health and hospice enrollments through shared owners, managing employees, addresses, suites, phones, faxes, authorised officials, EINs and ownership changes, producing 272,165 nodes and 469,115 edges. Owner identities are merged using PECOS associate IDs, exact name and ZIP keys, and a probabilistic matching model that only links pairs at 0.95 posterior probability when a location field also agrees, which linked 738 pairs. Very large hubs such as national chains and buildings with 25 or more tenants are held out so they do not merge everything into one component, and shared addresses and phones are down-weighted; the resulting 1,939 communities are scored on structure, geographic context and exclusion or termination labels, leaving 136 eligible for the ranked list. The structure-only score, which uses no label information, has a precision at 50 of 0.56 against a base rate of 0.354, but the section stresses that the top-10 figure rests on ten items and is not meaningful alone, and that the label set is incomplete because revoked providers have already left the enrollment file. California dominates the top 200 with 56 communities and 2,880 providers, while Florida shows the largest 2024 Medicaid dollars at 438.4 million; every ranked community is a referral candidate for records review, not a finding.
...
**In plain language.** This detector checks whether Medicaid paid claims with service months after a provider identifier was placed on a federal or state exclusion, revocation or termination list. Matching is exact by NPI, and each NPI-month of spending is counted once whether the NPI billed or rendered the service. Of 16,357 NPIs on a tier A list with an effective date inside the 2018-01 to 2024-12 window, 391 have Medicaid claims in later service months totalling $55.92M, and 197 of those were paid in six or more months after the action, worth $50.65M. Rows whose list name shares no token with the NPPES record are set aside as tier C and excluded from the headline, which removes 557 rows and $260.43M, and administrative Medicare revocations are kept only as tier B. The section states plainly that these are not improper payments, since a Medicare revocation is not by itself a Medicaid payment bar, appeals and reinstatements exist, and some amounts may have been recouped later; the cross-state termination list is described as a screening queue rather than a finding because state termination codes are uneven.
...
**In plain language.** This section combines every detector output into a single row per provider identifier, assigning one tier, one score and the reasons behind it. Tiers run from documented list actions followed by later payment down to informational, with base scores of 90, 75, 60, 45 and 25. The score adds 8 points for each extra strong finding that independently reached the same identifier, capped at 16, plus up to 9 points based on the log of dollars at risk, with the total capped at 100. Dollars at risk always come from the single detector that set the tier, never a sum and never borrowed from a weaker signal, so a lone volume flag cannot push a small case above a large documented one. The tiers hold 391, 604, 6,173, 1,999 and 2,111 identifiers with 56.96, 2,718.62, 1,138.73, 4,827.49 and 2,770.56 million dollars at risk respectively, and no identifier was reached independently by two or more detectors, which matters because corroboration is the strongest signal the pipeline produces. Every row listed is a candidate for records review, not a finding.
...
| 2 | impossible volume with concurrency | 604 | 2,718.62 | 0 |
| 3 | network structure with a list link | 6,173 | 1,138.73 | 0 |
| 4 | structure or single-organization volume | 1,999 | 4,827.49 | 0 |
| 5 | informational | 2,111 | 2,770.56 | 0 |
