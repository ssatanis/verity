...
## enforcement (DOJ API and HHS-OIG enforcement listing; daily)
- `data/enforcement/raw.jsonl`, loaded as `enforcement_raw`: one record per release. DOJ from `https://www.justice.gov/api/v1/press_releases.json` (fields uuid, title, body, date, topic[], component[], url); kept when the topic tag is Healthcare Fraud or the title names a health program or setting; `component` "USAO - State, District" gives the state. OIG from `https://oig.hhs.gov/fraud/enforcement/?page=N` (20 cards per page: date, title, category such as Criminal and Civil Actions or State Enforcement Agencies), each followed to its originating release (justice.gov, a state attorney general, a Medicaid fraud control unit) for the full text. Backfilled to 2024-01-01; `--daily` fetches the last seven days.
- `enforcement_actions` (one per originating release; DOJ and OIG copies collapse on the URL): relevant, action_type, tier (adjudicated, alleged, administrative: a fixed mapping from action_type), programs, scheme, dollars, action_date, confidence, model. `enforcement_parties`: name, kind, role, provider_type, city, state, is_provider, npi (only when printed).
- `enforcement_npi_matches`: party to NPPES candidate judgements; only same_entity with high confidence is read by Detectors 1 and 3 and the risk score. Every match is a name match by construction and is reported as `name_match_model_high`.
